...
void Display()
{
	...
	else
	{
		...
		for(temp=head; temp!=NULL; temp=temp->next)
		{
			printf("\n  %d    %s    %d   %d   %d   %d    %0.2f",temp->roll,temp->name,temp->s1,temp->s2,temp->s3,temp->total,temp->per);
		}
	}
}
void InserLast()
{
	if(head==NULL)
	{
		neawnode=(struct student *)malloc(sizeof(struct student));
		printf("\n Enter Roll No. :-");
		scanf("%d",&newnode->roll);
		newnode->next=NULL;
		printf("\n Enter Student Name :-");
	...
	{
			newnoede=(struct student *)malloc(sizeof(struct student));
			printf("\n Enter Roll No. :-");
			scanf("%d",&newnode->roll);
			newnode->next=NULL;
			printf("\n Enter Student Name :-");
